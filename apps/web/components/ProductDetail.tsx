...
import { Breadcrumb, FAQBlock, ReviewSummary, ScentProfile, Gallery, ProductGrid, type FaqItem } from "@homchalui/ui";
...
const SIMILAR_HEADING: Record<Locale, string> = { th: "สินค้าที่คล้ายกัน", en: "Similar products", zh: "相似商品" };
...
export async function ProductDetail({ slug, locale }: { slug: string; locale: Locale }) {
  ...
  const similar = await getSimilar(p.id, locale, 8);
  ...
  return (
    <>
      ...
      <RecentlyViewedRecorder href={sourcePage} name={t.name} image={p.mainImageUrl} />
      <main className="mx-auto max-w-6xl px-4 py-6">
        ...
        {similar.length > 0 && (
          <section className="mt-10">
            <h2 className="mb-4 text-lg font-semibold text-brand-dark">{SIMILAR_HEADING[locale]}</h2>
            <ProductGrid items={similar} locale={locale} listName={`similar:${p.id}`} sessionId={sessionId} />
          </section>
        )}
      </main>
